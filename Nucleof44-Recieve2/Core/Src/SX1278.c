...
int8_t SX1278_SNR_LoRa(SX1278_t *module) {
    // Read SNR register value (signed 8-bit value)
    int8_t snr = (int8_t)SX1278_SPIRead(module, LR_RegPktSnrValue);

    // The SNR value is in 0.25 dB steps, so divide by 4 to get dB
    return snr / 4;
}
